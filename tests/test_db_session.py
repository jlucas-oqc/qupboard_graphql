...
def patched_engine_setup(monkeypatch) -> tuple[dict[str, object], object, list[tuple[object, str, object]]]:
    ...
    sentinel_sync_engine = object()

    class SentinelEngine:
        sync_engine = sentinel_sync_engine
...
def test_get_engine_sets_sqlite_connect_args(patched_engine_setup):
    ...
    engine = session_module.get_engine("sqlite+aiosqlite:///tmp.db")

    assert engine is sentinel_engine
    assert captured["url"] == "sqlite+aiosqlite:///tmp.db"
    assert captured["kwargs"] == {"connect_args": {"check_same_thread": False}}
    assert listener_calls[0][1] == "connect"
    assert listener_calls[0][2] is session_module._set_sqlite_pragma
...
def test_get_engine_uses_empty_connect_args_for_non_sqlite(patched_engine_setup):
    ...
    engine = session_module.get_engine("postgresql+asyncpg://user:pass@localhost:5432/mydb")

    assert engine is sentinel_engine
    assert captured["url"] == "postgresql+asyncpg://user:pass@localhost:5432/mydb"
